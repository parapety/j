...
Vagrant.configure(2) do |config|
  ...
  # documentation for more information about their specific syntax and use.
	...
	SHELL
    config.vm.provision "tomcat", type: "shell", inline: <<-SHELL
        sudo apt-get install -y tomcat7
        cp /vagrant/vagrant_setup/tomcat7 /etc/default/tomcat7
        sudo service tomcat7 restart
        sudo apt-get install -y tomcat7-docs tomcat7-admin tomcat7-examples
        sudo apt-get install -y default-jdk
        sudo apt-get install -y ant
        cp /vagrant/vagrant_setup/tomcat-users.xml /etc/tomcat7/tomcat-users.xml
        sudo service tomcat7 restart
    SHELL
end
